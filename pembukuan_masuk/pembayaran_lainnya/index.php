<?php
  ...
  $page_title = "Pembayaran Lainnya";
  $breadcumb_name = ['Pembukuan Masuk','Pembayaran Lainnya'];
  $breadcumb_link = ['pembukuan_masuk','/pembukuan_masuk/pembayaran_lainnya/'];
  $limit = 100;
  $anggota = pembayaran_lainnya_find_all_index($limit);
  $cari_data = '';
  $hidden_th = '';
  $hidden_empty = 'hidden';
  $check_database = pembayaran_lainnya_find_all_index(1);
  ...
  if (is_post_request()) {
    redirect_to(url_for('/pembukuan_masuk/pembayaran_lainnya/show.php?id='.$_POST['cari-data']));
  }
 ...
 ?>
   ...
   <h1 id="page-title" class="border border-5 border-primary custom-round">PEMBAYARAN LAINNYA</h1>
   <div class="card card-wrapping">
     <div class="row">
       <div class="col"></div>
       <div class="col-4 col-sm-4">
         <div class="card card-menu" <?php echo $hidden_button; ?>>
           <button type="button" class="btn btn-outline-primary btn-menu" onclick="location.href = '<?php echo url_for('/pembukuan_masuk/pembayaran_lainnya/input.php'); ?>';">
     ...
     <div class="table-database" id="table-databse">
       ...
       <form class="d-flex" action="<?php echo url_for('/pembukuan_masuk/pembayaran_lainnya/index.php'); ?>" method="post">
        <input class="form-control me-2" type="search" placeholder="Cari Data" aria-label="Search" name="cari-data" value="<?php echo $cari_data; ?>">
       ...
       <table class="table table-striped table-bordered table-data table-hover">
         <tr class="bg-primary" <?php echo $hidden_th; ?>>
           <th>No</th>
           <th>Tanggal</th>
           <th>No Kwitansi/Kode Penerimaan</th>
           <th>Uraian</th>
           <th>Jumlah</th>
         ...
         while($data = mysqli_fetch_assoc($anggota)){?>
           <tr id="<?php echo 'list_no_'.$i; ?>" class="" data-href='url://'>
             ...
             <td><?php echo $data['no_kwitansi_penerimaan']; ?></td>
             <td><?php echo $data['uraian']; ?></td>
             <td><?php echo "Rp. ".number_format($data['jumlah'], '0', '', '.'); ?></td>
             <td <?php echo $hidden_button; ?>> <a href="<?php echo url_for('/pembukuan_masuk/pembayaran_lainnya/edit.php?id='.$data['id']); ?>">Edit/View</a> | <a href="<?php echo url_for('/pembukuan_masuk/pembayaran_lainnya/delete.php?id='.$data['id']); ?>">Delete</a> </td>
           </tr>
         <?php $i++; } ?>
       </table>
     </div>
   ...
   <script type="text/javascript" src="<?php echo url_for('/js/pembayaran_lainnya.js'); ?>">
