<?php
  ...
  $id = $_GET['id'];
  $page_title = "DELETE";
  $data = pembayaran_pajak_find_one($id);
  $delete_dir = PROJECT_PATH .'/uploads/pembukuan_keluar/pembayaran_pajak/'.$data['id'];
  $post_req = false;
  ...
  if (is_post_request()) {
    pembayaran_pajak_delete_one($id);
...
?>
  <div class="card card-wrapping">
    <form method="post" action="<?php echo url_for('/pembukuan_keluar/pembayaran_pajak/delete.php?id='.$id); ?>" <?php if ($post_req) {
      echo "hidden";
    } ?>>
      ...
      <div class="mb-3">
        ...
        <input class="form-control" type="text" aria-label="" name="nama" disabled value="<?php echo $data['jenis_pajak']; ?>">
      </div>
      <div class="mb-3">
