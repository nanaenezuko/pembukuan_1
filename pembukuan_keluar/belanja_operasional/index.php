<?php
  ...
  $page_title = "Belanja Operasional";
  $breadcumb_name = ['Pembukuan Keluar','Belanja Operasional'];
  $breadcumb_link = ['pembukuan_keluar','/pembukuan_keluar/belanja_operasional/'];
  $limit = 100;
  $anggota = belanja_operasional_find_all_index($limit);
  $cari_data = '';
  $hidden_th = '';
  $hidden_empty = 'hidden';
  $check_database = belanja_operasional_find_all_index(1);
  ...
  if (is_post_request()) {
    $anggota = belanja_operasional_search_data($_POST['cari-data'], $limit);
 ...
 ?>
   ...
   <h1 id="page-title" class="border border-5 border-primary custom-round">BELANJA OPERASIONAL</h1>
   <div class="card card-wrapping">
     <div class="row">
       <div class="col"></div>
       <div class="col-4 col-sm-4">
         <div class="card card-menu" <?php echo $hidden_button; ?>>
           <button type="button" class="btn btn-outline-primary btn-menu" onclick="location.href = '<?php echo url_for('/pembukuan_keluar/belanja_operasional/input.php'); ?>';">
     ...
     <div class="table-database" id="table-databse">
       ...
       <form class="d-flex" action="<?php echo url_for('/pembukuan_keluar/belanja_operasional/index.php'); ?>" method="post">
       ...
       <table class="table table-striped table-bordered table-data">
         ...
         while($data = mysqli_fetch_assoc($anggota)){?>
           <tr id="<?php echo 'list_no_'.$i; ?>">
             ...
             <td><?php echo $data['uraian_belanja']; ?></td>
             <td><?php echo "Rp. ".number_format($data['jumlah'],"0",",","."); ?></td>
             <td> <a href="<?php echo url_for('/pembukuan_keluar/belanja_operasional/edit.php?id='.$data['id']);?>" <?php echo $hidden_button; ?>>edit/view</a> | <a href="<?php echo url_for('/pembukuan_keluar/belanja_operasional/show.php?id='.$data['id']);?>" target="_blank">print</a> | <a href="<?php echo url_for('/pembukuan_keluar/belanja_operasional/delete.php?id='.$data['id']);?>" <?php echo $hidden_button; ?>>delete</a> </td>
           </tr>
         <?php $i++; } ?>
       </table>
     </div>
   ...
   <script type="text/javascript" src="<?php echo url_for('/js/belanja_operasional.js'); ?>">
