<?php
  ...
  $id = $_GET['id'];
  $page_title = "DELETE";
  $data = pembayaran_anggota_keluar_find_one($id);
  $delete_dir = PROJECT_PATH .'/uploads/pembukuan_keluar/pembayaran_anggota_keluar/'.$data['no_anggota'];
  $post_req = false;
  ...
  if (is_post_request()) {
    pembayaran_anggota_keluar_delete_one($id);
...
?>
  <div class="card card-wrapping">
    <form method="post" action="<?php echo url_for('/pembukuan_keluar/pembayaran_anggota_keluar/delete.php?id='.$id); ?>" <?php if ($post_req) {
      echo "hidden";
    } ?>>
