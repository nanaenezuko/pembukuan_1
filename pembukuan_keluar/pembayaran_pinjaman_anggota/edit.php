<?php
  ...
  if (is_post_request() && $id !== "") {
    $data = pembayaran_pinjaman_anggota_find_one($_GET['id']);

    $result = pembayaran_pinjaman_anggota_update($id);

    $file_name = [$data['no_anggota']."-formulir",
      $data['no_anggota']."-bukti-bayar"];

    $field_name = ["formulir", "bukti_bayar"];
        ...
        if ($upload_ok == 0 && !$file_upload[$i]['name'] !== "") {
          ...
          if ($file_upload[$i]['name'] !== ""){
            if (move_uploaded_file($file_upload[$i]["tmp_name"], $target_dir.$file_name[$i].".".$image_file_type[$i])) {
              ...
              $sql = "UPDATE pembayaran_pinjaman_anggota SET link_".$field_name[$i]." = '".$file_name[$i].".".$image_file_type[$i]."' ";
  ...
  if ($id !== "") {
    $data = pembayaran_pinjaman_anggota_find_one($_GET['id']);
    if (isset($data)) {
      $file_dir = "/uploads/pembukuan_keluar/pembayaran_pinjaman_anggota/".$data['no_anggota']."/".$data['folder']."/";
      $page_title = "Edit Data";
      $breadcumb_name = ['Pembukuan Keluar','Pembayaran Pinjaman Anggota','Edit'];
      $breadcumb_link = ['pembukuan_keluar','pembukuan_keluar/pembayaran_pinjaman_anggota/',
      '/pembukuan_keluar/pembayaran_pinjaman_anggota/edit.php'];
    } else {
 ...
 ?>
      ...
      <div class="card rounded">
        ...
        <div class="form-page">
          <form enctype="multipart/form-data" method="post" action="<?php echo url_for('/pembukuan_keluar/pembayaran_pinjaman_anggota/edit.php?id='.$id); ?>">
            <div class="mb-3">
            ...
            <div class="mb-3">
              ...
              <input id="input_lama_pinjaman" class="form-control <?php echo $valid; ?>" type="text" placeholder="Input Lama Pinjaman" aria-label="" name="lama-pinjaman" value="<?php echo $data['lama_pinjaman']; ?>" required>
            ...
            <div class="mb-3">
              ...
              <input id="input_jumlah_pinjaman" class="form-control <?php echo $valid ?>" type="number" placeholder="Input Jumlah Pinjaman" aria-label="" name="jumlah-pinjaman" value="<?php echo $data['jumlah_pinjaman']; ?>" required>
            ...
            <div class="mb-3">
              ...
              <div class="d-grid gap-2">
                <button id="btn-file-formulir" class="btn btn-outline-primary" type="button" onclick="location.href = '<?php echo url_for($file_dir.$data['link_formulir']); ?>';" <?php if (isset($error['file_upload_0'])) {
                  echo "hidden";
                } ?>>FILE FORMULIR</button>
              </div>
              <div id="file-formulir" class="" <?php if (!isset($error['file_upload_0'])) {
                echo "hidden";
              } ?>>
