...
if (page_title == "Input Anggota Keluar") {

  var input_no_anggota = document.getElementById("input_no_anggota");
  var input_jumlah_simpanan = document.getElementById("input_jumlah_simpanan");
  ...
  input_no_anggota.addEventListener("input", function(){
    ...
    getDataTotal(input_no_anggota.value, input_jumlah_simpanan);
  });
  
  input_text_validation(input_jumlah_simpanan);
}

function getDataTotal(no_anggota, total){
  if (no_anggota == "") {
    total.value = "";
    return;
  }
  ...
  xhttp.open("GET", "getjumlah.php?q="+no_anggota);
  xhttp.send();
}
...
function click_redirect_to(){
  window.location.href = '/pembukuan/keanggotaan/anggota_keluar/index.php';
}
